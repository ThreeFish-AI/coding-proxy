...
graph TD
    ...
    subgraph RW["release.yml<br/>Release / Publish to PyPI"]
        direction TB
        B1["Job: build<br/>Checkout → Python 3.13 → uv<br/>python -m build → twine check<br/>Upload artifact (14d)"]
...
|------|------------|--------------|---------------------|
| Python | `3.13` | `actions/setup-python@v5` | 对齐 [`pyproject.toml`](../pyproject.toml) 中 `requires-python = ">=3.13"` |
...
sequenceDiagram
    ...
    RW->>RW: Job: build (Python 3.13 + uv)
...
| ③ Build | Python 3.13 + uv + build deps → `python -m build` → `twine check` | 复现与 `release.yml` 完全一致的构建过程 |
...
| CI 配置 | 项目配置 | 对齐关系 |
|---------|---------|---------|
| `python-version: "3.13"` | `requires-python = ">=3.13"` in [`pyproject.toml`](../pyproject.toml) | CI 构建环境必须满足项目的最低 Python 版本要求 |
...
| 52–91 | `jobs.build` | 构建作业：checkout → Python 3.13 → uv → build → twine check → upload artifact |
...
| `python-version` | `"3.13"` | `"3.13"` | 必须与 `requires-python` 一致 |
