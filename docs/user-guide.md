...
coding-proxy 是一个面向 Claude Code 的多供应商智能代理服务。它在 Claude Code 和 API 供应商之间充当透明代理，具备以下核心能力：

- **N-tier 自动故障转移**：支持多层供应商链式降级（Anthropic → Copilot → Antigravity → 智谱 GLM），恢复后自动切回
...
- **弹性设施保护**：每层供应商独立配备熔断器、滑动窗口配额守卫（支持小时级与周级双窗口）、Rate Limit 精确截止控制
- **OAuth 认证管理**：内置 GitHub Device Flow 与 Google OAuth 登录流程，支持运行时重认证与凭证自动刷新

### 1.2 工作原理

```
Claude Code ──→ coding-proxy ──→ Tier 0: Anthropic API（主供应商）
                     │
                     ├──→ Tier 1: GitHub Copilot（中间层）
                     │
                     ├──→ Tier 2: Antigravity Claude（中间层）
                     │
                     └──→ Tier 3: 智谱 GLM API（终端层，无熔断器）
```

正常情况下，coding-proxy 将请求透传到 Anthropic API。当检测到限流、配额耗尽或服务过载等错误时，按优先级链自动降级到下一层供应商。每层独立配备熔断器和配额守卫——**未配置 `circuit_breaker` 的 vendor 自动成为终端层**（如智谱 GLM），始终接受请求且不触发进一步故障转移。供应商恢复后，代理会自动尝试切回更高优先级的层级。整个过程对用户透明，无需手动干预。
...
> 各端点的详细说明和响应格式参见[第 5 章](#5-http-api-端点)。
...
以下为精简版 `config.example.yaml`，作为唯一权威配置模板。完整注释版请直接参阅项目根目录下的 `config.example.yaml`。
...
vendors:
  - vendor: anthropic                    # 主供应商
    base_url: "https://api.anthropic.com"
    timeout_ms: 300000
    circuit_breaker:                     # 中间层标识
      failure_threshold: 3
      recovery_timeout_seconds: 300
      success_threshold: 2
      max_recovery_seconds: 3600
  ...
  - vendor: copilot                      # 中间层（默认禁用）
    ...
    # ... circuit_breaker / quota_guard / retry 同上 ...
...
tiers: ["anthropic", "copilot", "antigravity", "zhipu"]   # 可选

failover:
  status_codes: [429, 403, 503, 500]
  error_types: ["rate_limit_error", "overloaded_error", "api_error"]
  error_message_patterns: ["quota", "limit exceeded", "usage cap"]
