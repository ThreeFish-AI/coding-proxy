...
与此同时，多个 Anthropic 兼容或可转换的 API 通道为构建多供应商容灾体系提供了可能：
...
**coding-proxy 的核心诉求**：当任一供应商不可用时，自动、无缝地沿 N-tier 层级链降级到下一个可用供应商，对 Claude Code 客户端完全透明，用户无需手动干预。
...
| **N-tier 链式降级** | 支持 N 个供应商层级按优先级链式降级，每个层级独立配置弹性设施 |
...
graph TD
    ...
    subgraph T0["VendorTier 0: Anthropic"]
        direction LR
    ...
    subgraph T1["VendorTier 1: Copilot"]
        direction LR
        C_VE["CopilotVendor"]
        C_CB["CB"]
        C_QG["QG"]
    ...
    subgraph T2["VendorTier 2: Antigravity"]
        direction LR
    ...
    subgraph TN["VendorTier N: Zhipu（终端，无 CB/QG）"]
        Z_VE["ZhipuVendor"]
    end
...
| **vendors** | [`vendors/`](../src/coding/proxy/vendors/) | 供应商抽象基类与具体实现（Anthropic、Copilot、Antigravity、Zhipu） |
| **convert** | [`convert/`](../src/coding/proxy/convert/) | Anthropic ↔ Gemini 双向格式转换（请求、响应、SSE 流） |
| **routing** | [`routing/`](../src/coding/proxy/routing/) | N-tier 链式路由、VendorTier 聚合、熔断器、配额守卫、模型映射 |
...
**指数退避 (Exponential Backoff)**：每次从 HALF_OPEN 回退到 OPEN 时，恢复等待时间翻倍（`recovery_timeout *= 2`），上限为 `max_recovery_seconds`（默认 3600 秒）。避免对仍未恢复的供应商频繁重试。
...
- 代理在幕后完成供应商选择、故障转移、模型映射、用量记录等增值逻辑
...
VendorTier 将三个正交关注点聚合为路由器的最小调度单元：
...
**应用位置**：[`vendors/copilot.py`](../src/coding/proxy/vendors/copilot.py) — `CopilotTokenManager`；[`vendors/antigravity.py`](../src/coding/proxy/vendors/antigravity.py) — `GoogleOAuthTokenManager`
...
两者均支持**被动刷新**：当供应商返回 401/403 时，通过 `_on_error_status()` 调用 `invalidate()` 标记 token 失效，下次请求自动触发重新获取。
...
1. 主供应商返回成功（`status_code < 400`）→ `record_success()` → 返回
2. 主供应商返回错误 → 检查 `should_trigger_failover()`：
...
2. 为每个启用了 QuotaGuard 的 tier 加载基线：
   - `token_logger.query_window_total(window_hours, vendor=tier.name)` — 查询历史用量
   - `quota_guard.load_baseline(total_tokens)` — 初始化滑动窗口

**关闭 (Shutdown)**：
1. `router.close()` — 关闭所有供应商 HTTP 客户端（含 TokenManager 客户端）
...
**数据结构**（[`vendors/base.py`](../src/coding/proxy/vendors/base.py)）：
...
**AnthropicVendor**（Tier 0，[`vendors/anthropic.py`](../src/coding/proxy/vendors/anthropic.py)）：
- 构造：`AnthropicVendor(config: AnthropicConfig, failover_config: FailoverConfig)`
- 过滤 hop-by-hop 头（`host`、`content-length`、`transfer-encoding`、`connection`）
- 透传客户端的 OAuth token（`authorization` 头）和请求体
- 不修改请求体

**CopilotVendor**（Tier 1，[`vendors/copilot.py`](../src/coding/proxy/vendors/copilot.py)）：
- 构造：`CopilotVendor(config: CopilotConfig, failover_config: FailoverConfig)`
...
**AntigravityVendor**（Tier 2，[`vendors/antigravity.py`](../src/coding/proxy/vendors/antigravity.py)）：
- 构造：`AntigravityVendor(config: AntigravityConfig, failover_config: FailoverConfig)`
...
**ZhipuVendor**（终端 Tier N，[`vendors/zhipu.py`](../src/coding/proxy/vendors/zhipu.py)）：
- 构造：`ZhipuVendor(config: ZhipuConfig, model_mapper: ModelMapper)`（无 `failover_config`）
...
**VendorTier**（[`routing/tier.py`](../src/coding/proxy/routing/tier.py)）：
...
- 持有 `tiers: list[VendorTier]` + `token_logger`，支持 N-tier 链式降级
- `route_message()` → 非流式路由，按优先级逐层尝试，返回 `VendorResponse`
- `route_stream()` → 流式路由，按优先级逐层尝试，yield `(chunk, vendor_name)`
...
| 方法 | 说明 |
|------|------|
| `can_use_primary()` | 综合判断是否允许使用此供应商 |
...
|------|------|
| `log()` | 插入单条使用记录 |
| `query_daily(days, vendor, model)` | 按日期/供应商/模型聚合统计 |
| `query_window_total(window_hours, vendor)` | 查询滑动窗口内指定供应商的 token 总用量（供 QG 基线加载） |

**统计展示**（[`logging/stats.py`](../src/coding/proxy/logging/stats.py)）：
- `show_usage()` 使用 Rich Table 格式化展示统计结果
- 列：日期、供应商、请求模型、实际模型、请求数、输入 Token、输出 Token、故障转移数、平均耗时
...
**antigravity — Google Antigravity Claude 供应商配置**
...
| `vendors` | list[str] | 规则作用的供应商范围；留空时仅作用于 `fallback`/`zhipu`（兼容旧配置） |
| `target` | str | 目标模型名称 |
...
3. 按需覆写钩子方法：
   ...
   - `close()` — 释放额外资源
4. 在 `config/schema.py` 中添加供应商配置模型和对应的 CB/QG 配置字段
5. 在 `server/app.py` 的 `create_app()` 中构建 `VendorTier` 并插入到 `tiers` 链中
...
通过配置文件分别调整 `quota_guard` / `copilot_quota_guard` / `antigravity_quota_guard` 的参数，可为每个供应商独立配置配额管理策略：

```yaml
quota_guard:                # Anthropic 主供应商
  enabled: true
...
| `test_tier.py` | VendorTier 可执行判断、成功/失败记录、终端判定 |
| `test_vendors.py` | 请求头过滤、模型映射、故障转移判断、数据类默认值 |
| `test_copilot.py` | CopilotTokenManager 交换/缓存/过期/失效、CopilotVendor 请求准备 |
| `test_antigravity.py` | GoogleOAuthTokenManager 刷新/缓存/过期/失效、AntigravityVendor 格式转换+token 注入 |
...
| `test_token_logger.py` | 用量记录、窗口查询、按供应商/模型过滤 |
