...
def test_can_execute_no_cb_no_qg():
    """终端层（无 CB/QG）始终可执行."""
    tier = BackendTier(vendor=_make_backend())
...
def test_can_execute_cb_closed():
    """CB CLOSED 时可执行."""
    cb = CircuitBreaker()
    tier = BackendTier(vendor=_make_backend(), circuit_breaker=cb)
...
def test_can_execute_qg_exceeded():
    ...
    qg.notify_cap_error()
    tier = BackendTier(vendor=_make_backend(), quota_guard=qg)
    assert not tier.can_execute()
...
def test_can_execute_cb_ok_qg_exceeded():
    ...
    qg = QuotaGuard(enabled=True, token_budget=100, window_seconds=3600, probe_interval_seconds=99999)
    qg.notify_cap_error()
    tier = BackendTier(vendor=_make_backend(), circuit_breaker=cb, quota_guard=qg)
    assert not tier.can_execute()
...
def test_name_delegates_to_backend():
    tier = BackendTier(vendor=_make_backend("anthropic"))
...
def test_is_not_terminal_with_cb():
    """有 CB 非终端层."""
    tier = BackendTier(vendor=_make_backend(), circuit_breaker=CircuitBreaker())
...
def test_can_execute_weekly_qg_exceeded():
    ...
    tier = BackendTier(vendor=_make_backend(), weekly_quota_guard=wqg)
    assert not tier.can_execute()
...
def test_can_execute_qg_ok_weekly_qg_exceeded():
    ...
    wqg = QuotaGuard(enabled=True, token_budget=100, window_seconds=604800, probe_interval_seconds=99999)
    wqg.notify_cap_error()
    tier = BackendTier(vendor=_make_backend(), quota_guard=qg, weekly_quota_guard=wqg)
    assert not tier.can_execute()
