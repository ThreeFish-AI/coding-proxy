...
class TestEnforceAnthropicToolPairing:
    ...
    此函数替代原有多步串联管线（apply_anthropic_specific_fixes），
    ...
    def test_phase1_then_enforce_pairing(self):
        ...
        # Phase 1 重写了 ID，收集了 misplaced
        assert result.tool_id_map
        assert result.misplaced_tool_results

        # 在 deep copy 上执行 enforce_anthropic_tool_pairing（模拟 executor）
        body_copy = copy.deepcopy(result.body)
