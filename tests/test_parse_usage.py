...
def test_vendor_label_anthropic(caplog):
    ...
    parse_usage_from_chunk(
        _sse(
            '{"type":"message_delta","delta":{},"usage":{"output_tokens":42,"input_tokens":10}}'
        ),
        usage,
...
def test_vendor_label_gemini(caplog):
    ...
    parse_usage_from_chunk(
        _sse(
            '{"type":"message_delta","delta":{},"usage":{"output_tokens":30,"input_tokens":200}}'
        ),
        usage,
        vendor_label="Gemini",
    )
...
def test_no_vendor_label_omits_parenthesis(caplog):
    ...
    for record in caplog.records:
        assert (
            ...
            or "Extracted" not in record.message
            or ") " not in record.message[record.message.index("(") :]
        )
